...
langs = ['python', 'c#', 'PHP', 'Java', 'Go', 'JavaScript', 'PHP', 'C++', 'C', 'Kotlin', 'Delphi']
nums = [i for i in range(1, len(langs)+1)]

concut_list = list(zip(nums, [i.upper() for i in langs]))

def summ(x):
    return (sum([ord(str(i)) for i in x[1]]), x[1])

result_list = list(filter(lambda x: x[0] % 2 == 0, map(summ, concut_list)))
print(result_list)